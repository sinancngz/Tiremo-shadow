...
/* Same NS/KEY pattern as mqtt_ca/mqtt_cert/mqtt_key (ESP-AT proven). */
#define FLEET_MARKER_NS     "mqtt_devid"
#define FLEET_MARKER_KEY    "mqtt_devid"
/* Legacy markers from older builds. */
#define FLEET_MARKER_LEGACY_NS   "tiremo"
#define FLEET_MARKER_LEGACY_KEY  "provisioned"
#define FLEET_MARKER_LEGACY_VAL  "PROV1"
#define FLEET_MARKER_LEGACY2_NS  "mqtt_prov"
#define FLEET_MARKER_LEGACY2_KEY "devid"
...
static int fleet_sysmfg_read(char *resp, uint16_t respSize,
                             const char *ns, const char *key)
{
    char cmd[96];

    if ((resp == NULL) || (respSize < 32U) || (ns == NULL) || (key == NULL))
    {
        return -1;
    }
    ...
    (void)Wifi_Receive(resp, respSize, 3000U, POLLING_MODE);
...
{
    char resp[160];
    const char *deviceId = MqttIdentity_GetDeviceId();

    /* Primary: mqtt_devid / mqtt_devid == current MAC deviceIdentifier */
    if (fleet_sysmfg_read(resp, (uint16_t)sizeof(resp),
                          FLEET_MARKER_NS, FLEET_MARKER_KEY) == 0)
    {
        if (strstr(resp, deviceId) != NULL)
        {
            DebugFramework_PutsLine("[FLEET] marker OK for deviceId:");
            DebugFramework_PutsLine(deviceId);
            return 1U;
        }
        DebugFramework_PutsLine("[FLEET] marker deviceId mismatch — will re-provision");
        return 0U;
    }

    /* Older mqtt_prov/devid marker */
    if (fleet_sysmfg_read(resp, (uint16_t)sizeof(resp),
                          FLEET_MARKER_LEGACY2_NS, FLEET_MARKER_LEGACY2_KEY) == 0)
    {
        if (strstr(resp, deviceId) != NULL)
        {
            DebugFramework_PutsLine("[FLEET] legacy mqtt_prov marker OK");
            return 1U;
        }
    }

    /* Legacy PROV1 — no deviceId binding; force re-provision once. */
    if (fleet_sysmfg_read(resp, (uint16_t)sizeof(resp),
                          FLEET_MARKER_LEGACY_NS, FLEET_MARKER_LEGACY_KEY) == 0)
    {
        if (strstr(resp, FLEET_MARKER_LEGACY_VAL) != NULL)
        {
            DebugFramework_PutsLine("[FLEET] legacy PROV1 only — re-provision for MAC clientId");
            return 0U;
        }
    }

    DebugFramework_PutsLine("[FLEET] no deviceId marker — need fleet");
    return 0U;
}
...
static int fleet_set_provisioned_marker(char *buffer)
{
    ...
    if ((buffer == NULL) || (deviceId == NULL) || (deviceId[0] == '\0'))
    {
        return -1;
    }

    len = (uint16_t)strlen(deviceId);
    fleet_at_sync();

    DebugFramework_PutsLine("[FLEET] Saving deviceId marker:");
    DebugFramework_PutsLine(deviceId);

    if (Wifi_SysMfgWriteBinary(buffer, FLEET_MARKER_NS, FLEET_MARKER_KEY,
                               deviceId, len, POLLING_MODE) != FUNC_OK)
    {
        DebugFramework_PutsLine("[FLEET] deviceId marker write failed");
        DebugFramework_PutsLine(buffer[0] != '\0' ? buffer : "(empty)");
        return -1;
    }

    /* NVS commit settling time before read-back */
    if (port != NULL)
    {
        port->delay_ms(200U);
    }
...
int MqttFleet_UploadBootstrap(char *buffer, uint16_t bufSize)
{
    ...
    DebugFramework_PutsLine("[FLEET] Embedding bootstrap certs into ESP32...");
    if (Wifi_MqttCertsUpload2(buffer, POLLING_MODE) != FUNC_OK)
    {
        DebugFramework_PutsLine("[FLEET] Bootstrap embed failed");
        return -1;
    }
    DebugFramework_PutsLine("[FLEET] Bootstrap certs embedded in ESP NVS");
    return 0;
}
...
int MqttFleet_Run(char *buffer, uint16_t bufSize)
{
    ...
    if (Wifi_MqttCertsUpload2(buffer, POLLING_MODE) != FUNC_OK)
    {
        DebugFramework_PutsLine("[FLEET] Permanent embed to ESP failed");
        return -1;
    }
    DebugFramework_PutsLine("[FLEET] Permanent certs embedded in ESP NVS");

    /* SYSMFG is unreliable while MQTT link is up — disconnect first. */
