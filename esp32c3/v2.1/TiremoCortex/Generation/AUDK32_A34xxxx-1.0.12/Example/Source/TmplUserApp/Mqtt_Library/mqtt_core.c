...
static uint8_t s_sntp_done = 0U;       /* 1 after successful NTP this boot */
...
FUNC_StatusTypeDef Wifi_SysMfgWriteBinary(char *buffer, const char *ns, const char *key,
                                          ...
                                          MQTT_DataRecvModeTypeDef mode)
{
    ...
    if ((buffer == NULL) || (ns == NULL) || (key == NULL) || (data == NULL) || (len == 0U)) {
        return FUNC_FAIL;
    }
...
static FUNC_StatusTypeDef wifi_sysmfg_write_binary(char *buffer, const char *ns, const char *key,
                                                  ...
                                                  MQTT_DataRecvModeTypeDef mode)
{
    return Wifi_SysMfgWriteBinary(buffer, ns, key, data, len, mode);
}
...
uint8_t Wifi_IsStationConnected(char *buffer, const char *ssid)
{
    char ssidPat[48];
    MqttPort_Status st;
    ...
    memset(buffer, 0, 160);
    if (Wifi_SendCommand("AT+CWJAP?\r\n") != MQTT_PORT_OK)
    {
        return 0U;
    }

    st = Wifi_Receive(buffer, 160, 2000U, POLLING_MODE);
    if ((st != MQTT_PORT_OK) && (st != MQTT_PORT_TIMEOUT))
    {
        return 0U;
    }

    if (strstr(buffer, "+CWJAP:") == NULL)
    {
        return 0U;
    }
    ...
    {
        snprintf(ssidPat, sizeof(ssidPat), "+CWJAP:\"%s\"", ssid);
        if (strstr(buffer, ssidPat) == NULL)
        {
            /* Some ESP-AT builds: +CWJAP:<enc>,"ssid",... */
            snprintf(ssidPat, sizeof(ssidPat), "\"%s\"", ssid);
            if (strstr(buffer, ssidPat) == NULL)
            {
                return 0U;
            }
                                        ...
                                        MQTT_DataRecvModeTypeDef mode)
{
    ...
    sprintf(cmd, "AT+MQTTCONNCFG=0,%d,%d,\"\",\"\",%d,%d\r\n", keepAlive, cleanSession, qos, retain);
    checkCmd = Wifi_SendCommand(cmd);
...
FUNC_StatusTypeDef Wifi_MqttClean2(char *buffer, MQTT_DataRecvModeTypeDef mode)
{
    MqttPort_Status checkCmd;
    MqttPort_Status checkRcv;
    WIFI_RespMsgTypeDef checkResp;

    checkCmd = Wifi_SendCommand("AT+MQTTCLEAN=0\r\n");
...
FUNC_StatusTypeDef Wifi_MqttSub2(char *buffer, const char *topic, uint8_t qos, MQTT_DataRecvModeTypeDef mode)
{
    ...
    if (checkCmd == MQTT_PORT_OK) {
        /* Long topics / broker ACL errors need a larger RX window than 200. */
        checkRcv = Wifi_Receive(buffer, 256, WIFI_FUNCS_STD_TIMEOUT, mode);

        if (checkRcv == MQTT_PORT_OK || checkRcv == MQTT_PORT_TIMEOUT) {
            if (strstr(buffer, "OK") != NULL && strstr(buffer, "ERROR") == NULL)
                return FUNC_OK;
            if (strstr(buffer, "already subscribed") != NULL)
                return FUNC_OK;
            if (strstr(buffer, "ERROR") != NULL)
                return FUNC_RX_ERROR;
            if (checkRcv == MQTT_PORT_TIMEOUT)
                return FUNC_TIMEOUT;
            return FUNC_RX_ERROR;
        } else {
            return port_status_to_func(checkRcv);
...
FUNC_InitTypeDef MQTT_Init(MQTT_Config *config)
{
    ...
    /*
     * If STA already joined the configured AP, skip WiFi reset/reconnect.
     * If NTP already succeeded this boot, skip SET_TIME as well.
     */
    if (Wifi_IsStationConnected((char *)config->mqttPacketBuffer, config->wifiID) != 0U)
    {
        MQTT_DBG("[MQTT] WiFi already connected — skip reset/join\r\n");
        LED_WifiConnected(1);
        if (s_sntp_done != 0U)
        {
            MQTT_DBG("[MQTT] NTP already done — skip to MQTT config\r\n");
    ...
    {
        MQTT_DBG("[MQTT] WiFi not connected — full bring-up\r\n");
        s_sntp_done = 0U;
    ...
    while (MQTTInitCase != MQTT_INIT_STATE_END_CASE) {
        ...
        case MQTT_INIT_STATE_MQTT_USER_CONFIG:
            ...
            config->mode_mqtt = MQTT_TLS_MUTUAL;
