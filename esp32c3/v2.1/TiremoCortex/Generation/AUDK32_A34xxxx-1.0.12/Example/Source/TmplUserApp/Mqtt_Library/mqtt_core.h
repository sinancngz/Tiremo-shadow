...
#define MQTT_BROKER_ADDRESS             "iot.tiremo.ai"
#endif

#define WIFI_FUNCS_STD_TIMEOUT          5000    /* ms */
#define WIFI_MQTT_CONN_TIMEOUT_MS       45000U
#define WIFI_FUNCS_STD_BUFF_SIZE        90
#define MQTT_DATA_PACKET_BUFF_SIZE      5120
/* Large enough for AWS fleet create-cert JSON (PEM + key + token). */
#define MAC_ID_CHAR_SIZE                17
#define MAC_ID_UINT_SIZE                6
#define MQTT_CONN_MAX_TRY               3
#define MQTT_CONN_ONLINE_WAIT_S         60U
 ...
 * Tiremo uses only mutual TLS (verify server + client cert) = 5.
 */
typedef enum {
    MQTT_TLS_MUTUAL = 5
} MQTT_UserConfigSchemeTypeDef;

#define MQTT_TLS_4  MQTT_TLS_MUTUAL
...
/**
 * @brief  1 if station is associated (AT+CWJAP?). If ssid != NULL, must match.
 */
