...
int MqttCerts_SetPermanent(const char *certPem, const char *keyPem)
{
    size_t certLen;
    size_t keyLen;
    ...
    certLen = cert_strnlen(certPem, CERT_CLIENT_BUF_SIZE - 1U);
    keyLen  = cert_strnlen(keyPem, CERT_KEY_BUF_SIZE - 1U);
    if ((certLen < 32U) || (keyLen < 32U))
    {
        return -1;
    }

    memcpy(s_slots[1].buf, certPem, certLen);
    s_slots[1].buf[certLen] = '\0';
    s_slots[1].len = certLen;

    memcpy(s_slots[2].buf, keyPem, keyLen);
    s_slots[2].buf[keyLen] = '\0';
    s_slots[2].len = keyLen;
...
int     MqttCerts_SetPermanent(const char *c, const char *k) { (void)c; (void)k; return -1; }
